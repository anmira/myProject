...
public class FileUtils {
	...
	public List<Map<String, Object>> parseInsertFileInfo(BoardVO boardVO, 
		...
		/*
			...
			Iterator을 이용하여 Map에 있는 데이터들을 while문을 이용하여 순차적으로 접근.
		*/
	...
	public List<Map<String, Object>> parseUpdateFileInfo(BoardVO boardVO, String[] files, String[] fileNames, MultipartHttpServletRequest mpRequest) throws Exception{ 
		...
		while(iterator.hasNext()){ 
			multipartFile = mpRequest.getFile(iterator.next()); 
			if(multipartFile.isEmpty() == false){ //multipartFile이 비어있지 않으면 if문을 타게되는데 새로운 첨부파일이 등록되었을때 탐.
				originalFileName = multipartFile.getOriginalFilename(); 
				originalFileExtension = originalFileName.substring(originalFileName.lastIndexOf(".")); 
				storedFileName = getRandomString() + originalFileExtension; 
		...
		if(files != null && fileNames != null){ //files와 fileNames가 null이 아니면 for문을 탐.이 for문은 삭제할 파일의 파일 번호와 파일이름을 받음.
	...
	public static String getRandomString() { //32글자의 랜덤한 문자열(숫자포함)을 만들어 반환
